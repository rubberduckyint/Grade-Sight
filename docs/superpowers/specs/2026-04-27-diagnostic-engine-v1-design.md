...
- **Wrong-only analysis mode** — for graded papers, default the engine to only classify problems the teacher already marked wrong (read red X's / score deductions from the page). Major cost + latency win when the teacher has already graded. UI checkbox "review the whole quiz/test" to opt back into full analysis. Folds naturally into Spec 12 because the answer-key flow is intrinsically wrong-only (key tells us what's wrong; engine just classifies why).
- **Printable corrections (PDF)** — teacher-launch requirement (Spec 13 candidate). Server-side PDF generation (`weasyprint` or `reportlab`), endpoint `GET /api/assessments/{id}/corrections.pdf`, "Download corrections" button on the detail page when status=`completed`. Layout: per-page header (student name, assessment date), one entry per wrong problem with student answer (struck through) + correct answer + pattern + step-by-step solution, footer "Generated by Grade-Sight". Optional: page thumbnails for stapling reference.
